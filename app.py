...
st.set_page_config(page_title="Stock Analysis with MAMA and FAMA", layout="wide")
...
st.markdown(
    ...
    h2, h3, .stMarkdown p, .stTextInput>div>div>input, .stDateInput>div>div>input, .stSelectbox>div>div>div {
        color: #0073e6;
    }
    .stSlider>div>div>div>div, .stSlider>div>div>div>span {
        color: white;
    }
    .stButton>button {
        color: #ffffff;
    ...
    .stPlotlyChart .plotly .xtick > text, .stPlotlyChart .plotly .ytick > text {
...
# Input fields for optimization date range
